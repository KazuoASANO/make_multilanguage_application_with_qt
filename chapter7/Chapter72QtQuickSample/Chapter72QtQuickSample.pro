...
# Chapter7 - 7.2 lupdate の自動実行 - Qt Quick Sample
#
# \note
...
# lrelease の自動実行 と 実行ファイルに Qt 翻訳バイナリファイル( .qm )を含める
CONFIG += lrelease embed_translations
# 自動生成されるリソースファイル
# ( qmake_qmake_qm_files.qrc )へ付与するプレフィックス
...
# lupdate の実行フルパスを LUPDATE に取得
qtPrepareTool(LUPDATE, lupdate)
lupdate.name = lupdate
lupdate.input = TRANSLATIONS EXTRA_TRANSLATIONS
lupdate.output = $$PWD/${QMAKE_FILE_IN_BASE}_ts
lupdate.commands = $$LUPDATE $$PWD/*.pro
# リンク先のオブジェクトのリストに出力を追加せず、 clean コマンドも追加しない
lupdate.CONFIG = no_link no_clean
QMAKE_EXTRA_COMPILERS += lupdate
# target_predeps へ build step を追加
lupdate.CONFIG += target_predeps no_clean
